...
const apifactura = new Vue({
    el: '#apifactura',
        ...
        methods: {
            cargarEstadofactura: function(factura) {
                this.factura = factura;
                this.estado_factura = factura.estado_id;
            },
            updateEstadofactura: function() {
                //actualizar estado de factura segun el id
                ...
                let datos = {
                    'id': this.factura.id,
                    'nuevoEstado': this.estado_factura
                };

                axios.put('/api/actualizarfactura',datos, {headers} ).then(function (response) {
                    if (response.status) {
                        window.location.href = '/admin/factura';
                    }
                })
	        ...
	        save: function() {
                if (!this.checkForm()) {
    	            ...
    	            axios.post(url, datos, {headers}).then(r => {
    	                if (r.data.response) {
    	                	window.location.href = '/admin/factura/create';
    	                }else{
